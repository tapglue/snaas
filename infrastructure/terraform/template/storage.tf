...
resource "aws_sqs_queue" "endpoint-change-dlq" {
    delay_seconds               = 0
    max_message_size            = 262144
    message_retention_seconds   = 1209600
    name                        = "endpoint-change-dlq"
    receive_wait_time_seconds   = 1
    visibility_timeout_seconds  = 300
}

resource "aws_sqs_queue" "endpoint-change" {
    delay_seconds               = 0
    max_message_size            = 262144
    message_retention_seconds   = 1209600
    ...
    redrive_policy              = <<EOF
{
    "deadLetterTargetArn": "${aws_sqs_queue.endpoint-change-dlq.arn}",
    "maxReceiveCount": 10
}
EOF
    visibility_timeout_seconds  = 60
